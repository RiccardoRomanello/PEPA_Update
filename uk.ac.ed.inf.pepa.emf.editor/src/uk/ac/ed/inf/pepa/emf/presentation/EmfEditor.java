...
import uk.ac.ed.inf.pepa.emf.provider.EmfItemProviderAdapterFactory;
 ...
 * @generated
 */
public class EmfEditor extends MultiPageEditorPart implements
		IProcessAlgebraEditor, IEditingDomainProvider, ISelectionProvider, IMenuListener, IViewerProvider, IGotoMarker {
	/**
	 ...
	 * @generated
	 */
	protected Collection<ISelectionChangedListener> 
	...
	public EmfEditor() {
		...
		factories.add(new EmfItemProviderAdapterFactory());
	...
	private static String getString(String key) {
		return PepaEditorPlugin.INSTANCE.getString(key);
	}
	...
	private static String getString(String key, Object s1) {
		return PepaEditorPlugin.INSTANCE.getString(key, new Object [] { s1 });
	}
	...
	/**
	 * Returns the first resource containing XMI data about the model.
	 * When other models are added to the editor for manipulation purposes,
	 * they are ignored by this method. If we want, we can disable the
	 * loadResource action which can be found in the superclass.
	 *  
	 */
	public IPepaModel getProcessAlgebraModel() {
		// FIXME Make it better, consider multiple resources, etc.
		Resource res = (Resource) editingDomain.getResourceSet().getResources()
				.get(0);
		URI uri = res.getURI();
		// TODO Find a method which maps the platform URI to a workspace URI
		// Here string manipulation only happens.
		...
		IPepaModel model = PepaCore.getDefault().getPepaManager().getModel(file);
