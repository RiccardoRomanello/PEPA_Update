...
//                System.out.println("setting initial value for species \"" + pDynamicSymbols[i].getName() + "\" to " + pDynamicSymbolValues[i]);
                                            ...
                                            String []pRequestedSymbolNames) throws DataNotFoundException, IllegalStateException, IllegalArgumentException, AccuracyException
    {
        ...
        // 6.4.07 Added to allow removal of if statement in Simulator.addRequestedSymbolValues - ajd
        double[] tArray;
        for(int i = 0; i < pNumResultsTimePoints; i++) {
        	tArray = new double[numRequestedSymbols];
        	for(int i2 = 0; i2 < numRequestedSymbols; i2++)
        		tArray[i2] = 0.00;
        ...
        double time = 0.0, previousTime = 0.00;
        ...
        // Next seven lines are for statistics. Code by Luca
        Object []statCollector = new Object[pNumResultsTimePoints];
        for(int timeCtr = 0; timeCtr < pNumResultsTimePoints; timeCtr++) {
        	TallyStore []stat = new TallyStore[numRequestedSymbols];
        	for(int symCtr = 0; symCtr < numRequestedSymbols; symCtr++)
        		stat[symCtr] = new TallyStore();
        	statCollector[timeCtr] = stat;
        }
        
        // Next six lines for reaction firing statistics. AJD
        ...
        double deadlockTime;
        LinkedList deadlockList = new LinkedList();
        ...
        while( --simCtr >= 0 )
        {
            ...
            // time = pStartTime; AJD, trying to implement burn in
            time = 0;// Handles burn in time for simulation 

            // prepareForSimulation(pStartTime); AJD, trying to implement burn in
            prepareForSimulation(0);// Handles burn in time for simulation 
            ...
            // prepareForStochasticSimulation(pStartTime, pSimulatorParameters); AJD, trying to implement burn in
            prepareForStochasticSimulation(0,pSimulatorParameters);// Handles burn in time for simulation 
            
            deadlockTime = -1;

            while(pNumResultsTimePoints - timePointIndex > 0)
            {
            	previousTime = time; // To calculate reaction times. AJD
                ...
                try
                { // Update reaction firing stats. AJD
                	if(time >= pStartTime) {
                		reactionCount[lastReactionIndex.getValue()]++;
                		reactionTime[lastReactionIndex.getValue()] += (time-previousTime);
                	}
                }
                catch(ArrayIndexOutOfBoundsException e) { /* do nothing - no reaction involved (null reaction) */ }
                ...
                if(doUpdates)
                {
                    ...
                    if(currentTimeMilliseconds - timeOfLastUpdateMilliseconds >= minNumMillisecondsForUpdate)
                    {
                        if(null != simulationController)
                        {
                            ...
                            if(isCancelled || simulationController.getFinished())
                            {
                                break;
                            }
                        }
        ...
        if(! isCancelled)
        {
            ...
            for(int i = 0; i < reactions.length; i++) {
            	reactionNames[i] = reactions[i].getName();
            	avgReactionCount[i] = reactionCount[i]*ensembleMult;
            	reactionTime[i] *= ensembleMult;
            }
    ...
    public SimulatorParameters getDefaultSimulatorParameters()
    {
        ...
        sp.setConfidenceInterval(DEFAULT_CONFIDENCE_INTERVAL);
