...
public abstract class Simulator 
{
    ...
    private final void checkReactionRates() throws DataNotFoundException
    {
        ...
        for(int reactionCtr = 0; reactionCtr < numReactions; ++reactionCtr)
        {
            ...
            computeReactionRate(reactionCtr);
    ...
    /* Overloaded method for statistical purposes
     * Luca Cacchiani 01/02/07
     */
    protected final SimulationResults createSimulationResults(double pStartTime,
            												  double pEndTime,
            												  SimulatorParameters pSimulatorParameters,
            												  ...
            												  Object []pResultsSymbolValues,
            												  double []pResultsFinalSymbolFluctuations,
            												  Object []pStatCollector,
            												  String[] pReactionNames,
            												  double[] pReactionCounts,
            												  double[] pReactionTimes)
    {
    	SimulationResults simulationResults = createSimulationResults(pStartTime, pEndTime, pSimulatorParameters, pResultsSymbolNames, pResultsTimeValues, pResultsSymbolValues, pResultsFinalSymbolFluctuations);
    ...
    protected final int addRequestedSymbolValues(double pCurTime,
    											 ...
    											 Object []pStatCollector) throws DataNotFoundException
    {
    	int numTimePoints = pTimeValues.length;
    	int numRequestedSymbolValues = pRequestedSymbols.length;
    	...
    	for(; timeCtr < numTimePoints; ++timeCtr)
    	{
    		...
    		{
    			double []symbolValues = (double []) pRetSymbolValues[timeCtr];
    			TallyStore []stat = (TallyStore []) pStatCollector[timeCtr]; // Luca
    			/* Never null now - ajd
    			if(null == symbolValues)
    			{
    				symbolValues = new double[numRequestedSymbolValues];
    			...
    			for(int symCtr = numRequestedSymbolValues - 1; symCtr >= 0; --symCtr)
    			{
    				Symbol symbol = pRequestedSymbols[symCtr];
    				symbolValues[symCtr] += mSymbolEvaluator.getValue(symbol);
    				stat[symCtr].add(mSymbolEvaluator.getValue(symbol)); // Luca
    			}
    		}
    		else
    		{
    ...
    protected final static double []createodeToJavaTimesArray(double pStartTime,
                                                     double pEndTime,
                                                     int pNumTimePoints)
    {
        ...
        if(pStartTime == 0.00)
        	...
        	offset = 1;
        	retTimesArray[0] = 0.00;
        }

        double deltaTime = (pEndTime - pStartTime) / ((pNumTimePoints - 1));
        
        
        for(int timeCtr = 0; timeCtr < pNumTimePoints; ++timeCtr)
        {
            double time = (timeCtr * deltaTime) + pStartTime; // They forgot to add initial time AJD
            if(time > pEndTime)
            {
                time = pEndTime;
            }
...
return(retTimesArray);
                                                                      ...
                                                                      int pNumResultsTimePoints)
    {
        ...
        if(stepSize > (1.0 / (pNumResultsTimePoints)))
        {
        ...
        if(hasDelayedReactionSolvers())
        {
            ...
            double maxStepSizeDueToDelayedReactions = getMinDelayedReactionDelay()/(numHistoryBins);
