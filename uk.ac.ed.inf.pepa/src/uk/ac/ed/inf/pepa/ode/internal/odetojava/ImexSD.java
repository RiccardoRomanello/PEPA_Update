...
public class ImexSD {
	...
	public static void imex_sd(ODE function, Span tspan, double[] u0,
			double h0, Btableau butcher, double atol, double rtol,
			String fileName, String stats, IWriterCallback writer)
			throws DifferentialAnalysisException, InterruptedException {
		ImexSD imexsd = new ImexSD(function, tspan, u0, h0, butcher, atol,
				rtol, fileName, stats, writer); // initialize the IMEX with Step
		// Doubling object
	 ...
	 * an interval of (temporal) integration, an initial value, an intial
	 * stepsize, a Butcher tableau, two tolerances, and a few strings for
	 * special features
	 */
	public ImexSD(ODE function, Span tspan, double[] u0, double h0,
			Btableau butcher, double atol, double rtol, String fileName,
			String stats, IWriterCallback writer) {
		// span check
		if (writer == null)
		...
		if (h0 <= 0) // error testing for h0 (must be greater than zero)
		{
			throw new IllegalArgumentException(
					"Initial stepsize must be greater than zero");
		}

		if (atol <= 0) // error testing for atol (must be greater than zero)
		{
			throw new IllegalArgumentException(
					"Abolute tolerance must be greater than zero");
		}

		if (rtol <= 0) // error testing for rtol (must be greater than zero)
		{
			throw new IllegalArgumentException(
					"Relative tolerance must be greater than zero");
		}

		// parameter independent initializations

		p = 3; // p = 3 is a good choice for most cases (IMEX443 and ones like
		// it)
	...
	public void routine() throws DifferentialAnalysisException,
		...
		double[] diff1 = new double[n]; // temporary array for difference of 2
		// arrays
		...
		while (!done) {
			...
			eta2 = imex1s.doOneStep(f, told + h / 2, eta2temp, h / 2); // with
			// h/2
			...
			double[] estimation = ErrorEstimator.stepdoubling_estimate(h, eta1,
					eta2, utemp, atol, rtol, p, amax, alpha);

			hNew = estimation[0]; // get required information from estimate
			hopt = estimation[1];
			norm = estimation[2];

			if (Double.isNaN(norm)) // check to see if norm is NaN, if
			...
			{ // step doubling and program is done
				if ((h / hopt) > 3) // in the very odd case that the stretched
				// step
				{ // is rejected we:
					...
					h = h / 2; // we cut h in half, seeing as neither half will
					// be too small
					...
					writer.writeToFile(told, uold_e2); // output new solution
					// array thus far (into
					// file)
				}
			} else if (lastStep || ((h / hopt) <= 3)) // if on our last step
			{ // (where step is usually very small) or optimal h is close
				StdMet.arraycpy(uold_e1, uold_e2); // enough to h, accept step
				// and
				h = hNew; // increment related counters
				naccept++;

				writer.writeToFile(told, uold_e2); // output new solution thus
				// far (into file)
			...
			} else // else we reject the step and increment related counters
			{
				...
				if (stats_on) // output statistics (if user has chosen so), note
				// that
