...
public class FrameworkBasedParallelBuilder implements IStateSpaceBuilder {
		...
		public void run() {
			...
			while (true) {
				...
				for (int i = 0; i < transitions.length; i++) {
					...
					uk.ac.ed.inf.pepa.ctmc.derivation.common.State state = new uk.ac.ed.inf.pepa.ctmc.derivation.common.State(
							aT.fTargetProcess, hashCode);
					
					uk.ac.ed.inf.pepa.ctmc.derivation.common.State old = map.putIfAbsent(state, state);
					if (old == null) {
						aT.fState = state;
	...
	public FrameworkBasedParallelBuilder(IStateExplorer[] explorers,
		...
		this.map = new StateConcurrentHashMap(200, 0.75f,
				explorers.length);
